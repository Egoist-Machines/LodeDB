...
def canonical_sha256(value: Any) -> str:
    """Hashes one JSON value using a stable, whitespace-free representation."""
...
def sha256_file(path: str | Path) -> str:
    """Streams one artifact digest without materializing it in memory."""

    digest = hashlib.sha256()
    with Path(path).open("rb") as file:
        while chunk := file.read(4 << 20):
            digest.update(chunk)
...
def _identity_fields(manifest: dict[str, Any]) -> tuple[str, str | None]:
    """Derives the corpus and evaluation identities from manifest provenance."""

    hashes = manifest["sha256"]
    corpus_id = canonical_sha256(
        {
            "schema": "wiki-dpr-corpus-v2",
            "rows": manifest["rows"],
            "dim": manifest["dim"],
    ...
    if hashes["gt_indices"] is None:
        return corpus_id, None
    evaluation_id = canonical_sha256(
        {
            "schema": "wiki-dpr-evaluation-v2",
            "corpus_id": corpus_id,
            "n_queries": manifest["n_queries"],
            "seed": manifest["seed"],
            "query_row_indices": manifest["query_row_indices"],
            "queries_sha256": hashes["queries"],
            "gt_indices_sha256": hashes["gt_indices"],
            "gt_scores_sha256": hashes["gt_scores"],
        }
    )
    return corpus_id, evaluation_id


def _with_identity_fields(manifest: dict[str, Any]) -> dict[str, Any]:
    manifest = json.loads(json.dumps(manifest))
    corpus_id, evaluation_id = _identity_fields(manifest)
...
def _atomic_save_npy(path: Path, array: np.ndarray) -> None:
    """Writes one generation-addressed NPY payload before it can be referenced."""
...
def load_manifest(data_dir: str | Path) -> dict[str, Any]:
    ...
    for key in ("base", "queries", "gt_indices", "gt_scores"):
        ...
        if files[key] is not None and (
            not isinstance(files[key], str)
            or not files[key]
            or Path(files[key]).name != files[key]
        ):
    ...
    for key in ("base", "queries"):
        _require_digest(hashes.get(key), f"manifest sha256.{key}")
    for key in ("gt_indices", "gt_scores"):
        _require_digest(hashes.get(key), f"manifest sha256.{key}", optional=True)
    if (hashes.get("gt_indices") is None) != (hashes.get("gt_scores") is None):
        raise ValueError("manifest ground-truth hashes must be both present or both null")
    ...
    if not isinstance(source.get("dataset"), str) or not source["dataset"]:
        raise ValueError("manifest source.dataset must be a non-empty string")
    if not isinstance(source.get("revision"), str) or not source["revision"]:
        raise ValueError("manifest source.revision must be a non-empty immutable identifier")
    if source["dataset"] != "synthetic" and (
        len(source["revision"]) not in (40, 64)
        or any(character not in "0123456789abcdef" for character in source["revision"])
    ):
        raise ValueError("manifest source.revision must be an immutable commit digest")
    if not isinstance(source.get("shards"), (list, type(None))):
        raise ValueError("manifest source.shards must be a list or null")
    ...
    if manifest.get("corpus_id") != corpus_id:
        raise ValueError("manifest corpus_id does not match its provenance")
    if manifest.get("evaluation_id") != evaluation_id:
        raise ValueError("manifest evaluation_id does not match its queries and ground truth")
    return manifest
...
def write_manifest(data_dir: str | Path, manifest: dict[str, Any]) -> Path:
    ...
    manifest = _with_identity_fields(manifest)
...
def validate_dataset(
    data_dir: str | Path, *, require_gt: bool = True, verify_base_hash: bool = False
) -> dict[str, Any]:
    ...
    if verify_base_hash and sha256_file(base_path) != manifest["sha256"]["base"]:
        raise ValueError("base file SHA-256 does not match the committed dataset manifest")
    ...
    if sha256_file(query_path) != manifest["sha256"]["queries"]:
        raise ValueError("queries file SHA-256 does not match the committed dataset manifest")
    ...
    if gt_indices_path is not None and gt_scores_path is not None:
        ...
        if sha256_file(gt_indices_path) != manifest["sha256"]["gt_indices"]:
            raise ValueError("ground-truth indices SHA-256 does not match the manifest")
        if sha256_file(gt_scores_path) != manifest["sha256"]["gt_scores"]:
            raise ValueError("ground-truth scores SHA-256 does not match the manifest")
    return manifest
...
def compute_exact_ground_truth(
    ...
    indices_name = f"{Path(GT_INDICES_NAME).stem}-{generation}.npy"
    scores_name = f"{Path(GT_SCORES_NAME).stem}-{generation}.npy"
    indices_path = data_dir / indices_name
    scores_path = data_dir / scores_name
    _atomic_save_npy(indices_path, best_indices)
    _atomic_save_npy(scores_path, best_scores)
    manifest["files"]["gt_indices"] = indices_name
    manifest["files"]["gt_scores"] = scores_name
    manifest["sha256"]["gt_indices"] = sha256_file(indices_path)
    manifest["sha256"]["gt_scores"] = sha256_file(scores_path)
...
def make_synthetic_dataset(
    ...
    base = np.memmap(base_path, dtype="<f4", mode="w+", shape=(rows, dim))
    ...
    manifest: dict[str, Any] = {
        ...
        "source": {
            "dataset": "synthetic",
            "revision": f"wiki-dpr-synthetic-v2-seed-{seed}",
            "shards": None,
        },
        "files": {
            "base": base_name,
