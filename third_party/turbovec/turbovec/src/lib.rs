...
impl TurboQuantIndex {
    ...
    pub fn bytes_per_vector(&self) -> Option<usize> {
        self.dim.map(|dim| dim * self.bit_width / 8)
    }

    /// Whether a data-dependent TQ+ calibration was fitted, as opposed to
    /// the identity calibration committed when the first add had fewer
    /// than `TQPLUS_MIN_SAMPLES` rows (identity is stored as explicit
    /// shift=0/scale=1 arrays) or the empty calibration of a pre-TQ+ load.
    /// A trickle-ingested index that grew past the threshold with identity
    /// calibration permanently misses the TQ+ recall lift; callers can
    /// surface a rebuild recommendation from this flag.
    ///
    ...
    pub fn calibration_fitted(&self) -> bool {
        ...
        let identity = self.tqplus_shift.iter().all(|value| *value == 0.0)
            && self.tqplus_scale.iter().all(|value| *value == 1.0);
        !identity
    }

    /// FNV-1a fingerprint over the calibrated coordinate system: `dim`,
    /// `bit_width`, and the frozen TQ+ shift/scale bit patterns. Encoded
    /// rows exported from one index are only valid for import into an
    /// index with an equal fingerprint (the rotation is deterministic in
    /// `dim`, so it needs no separate component).
    ///
    /// Local appliance extension on top of upstream v0.9.0.
    pub fn calibration_fingerprint(&self) -> u64 {
        ...
        for value in &self.tqplus_shift {
            mix(&value.to_bits().to_le_bytes());
        }
        for value in &self.tqplus_scale {
            mix(&value.to_bits().to_le_bytes());
        }
        hash
    }

    /// Encode `n` vectors into packed codes and scales using the index's
    /// frozen coordinate system (rotation, codebook, TQ+ calibration)
    /// without appending them. The caller guarantees the index has at
    /// least one stored vector, which implies the calibration is
    /// committed (small first adds and pre-TQ+ loads store explicit
    /// identity arrays), so encoding here can never refit global state.
    ///
    /// Local appliance extension on top of upstream v0.9.0.
    pub(crate) fn encode_rows(&self, vectors: &[f32], n: usize, dim: usize) -> (Vec<u8>, Vec<f32>) {
        assert!(
            !self.tqplus_shift.is_empty(),
            "encode_rows requires a committed calibration (add rows first)"
        );
        let rotation = self
            .rotation
            .get_or_init(|| rotation::make_rotation_matrix(dim));
        ...
        let centroids = self
            ...
            .get()
            .expect("centroids cache is initialized");
        let existing = Some((self.tqplus_shift.as_slice(), self.tqplus_scale.as_slice()));
        let (packed, scales, _, _) = encode::encode(
            vectors,
            n,
            dim,
            rotation,
    ...
    /// Borrow the packed code bytes and scale stored in `slot`.
    ///
    /// Local appliance extension on top of upstream v0.9.0.
    pub(crate) fn export_row(&self, slot: usize) -> (&[u8], f32) {
        let bytes_per_vec = self
            .bytes_per_vector()
            .expect("export_row on an index with no committed dim");
        let start = slot * bytes_per_vec;
        (&self.packed_codes[start..start + bytes_per_vec], self.scales[slot])
    }

    /// Decode the row stored in `slot` into rotated-coordinate float space,
    /// scaled by the stored per-row scale, writing `dim` values into `out`.
    ///
    /// The decoded row `y` is exactly the vector the search kernel scores
    /// queries against (before the kernel's uint8 LUT quantization):
    ///
    /// ```text
    /// y[d] = scale * (centroids[code[d]] / scale_tq[d] - shift[d])
    /// score(q) = <q @ rotation^T, y>
    /// ```
    ///
    /// so an exact inner product between a rotated query and `y` reproduces
    /// the kernel's calibrated score estimate without LUT quantization
    /// error. Empty TQ+ arrays (pre-TQ+ loads) behave as identity.
    ///
    /// Local appliance extension on top of upstream v0.9.0 — see
    /// `LOCAL_PATCHES.md` at the repository root.
    pub(crate) fn reconstruct_row_into(&self, slot: usize, out: &mut [f32]) {
        let dim = self
            .dim
            .expect("reconstruct_row_into on an index with no committed dim");
        debug_assert_eq!(out.len(), dim);
        let centroids = self.centroids.get_or_init(|| {
            let (_, c) = codebook::codebook(self.bit_width, dim);
            c
        });
        let (row, scale) = self.export_row(slot);
        let bytes_per_plane = dim / 8;
        let identity_calibration = self.tqplus_scale.is_empty();
        for (d, out_value) in out.iter_mut().enumerate() {
            let byte_pos = d / 8;
            let bit_pos = 7 - (d % 8);
            let mut code = 0usize;
            for plane in 0..self.bit_width {
                if row[plane * bytes_per_plane + byte_pos] & (1 << bit_pos) != 0 {
                    code |= 1 << plane;
                }
            }
            let centroid_in_orig = if identity_calibration {
                centroids[code]
            } else {
                centroids[code] / self.tqplus_scale[d] - self.tqplus_shift[d]
            };
            *out_value = scale * centroid_in_orig;
        }
    }

    /// A copy of the deterministic rotation matrix (`dim`, `dim`,
    /// row-major), or `None` before the dim commits. Queries are rotated as
    /// `q_rot = q @ rotation^T` (exactly the GEMM the search path runs), so
    /// callers scoring against [`Self::reconstruct_row_into`] output must
    /// apply the same transform.
    ///
    /// Local appliance extension on top of upstream v0.9.0.
    pub fn rotation_matrix_copy(&self) -> Option<Vec<f32>> {
        let dim = self.dim?;
        Some(
            self.rotation
                .get_or_init(|| rotation::make_rotation_matrix(dim))
    ...
    /// Overwrite the packed code bytes and scale stored in `slot` and
    /// invalidate the derived blocked cache. The caller validates lengths.
    ///
    /// Local appliance extension on top of upstream v0.9.0.
    pub(crate) fn overwrite_row(&mut self, slot: usize, codes: &[u8], scale: f32) {
        let bytes_per_vec = self
            .bytes_per_vector()
            .expect("overwrite_row on an index with no committed dim");
        debug_assert_eq!(codes.len(), bytes_per_vec);
        let start = slot * bytes_per_vec;
        self.packed_codes[start..start + bytes_per_vec].copy_from_slice(codes);
        self.scales[slot] = scale;
        self.blocked = OnceLock::new();
    }

    /// Append one pre-encoded row, returning its new slot, and invalidate
    /// the derived blocked cache. The caller validates lengths and that the
    /// calibrated coordinate system matches the exporting index.
    ///
    /// Local appliance extension on top of upstream v0.9.0.
    pub(crate) fn append_encoded_row(&mut self, codes: &[u8], scale: f32) -> usize {
        let bytes_per_vec = self
            .bytes_per_vector()
            .expect("append_encoded_row on an index with no committed dim");
        debug_assert_eq!(codes.len(), bytes_per_vec);
        let slot = self.n_vectors;
        self.packed_codes.extend_from_slice(codes);
