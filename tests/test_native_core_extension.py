...
def test_native_core_extension_executes_vector_store_flow() -> None:
    assert native_core.native_core_abi_version() == 8
...
def test_native_core_extension_executes_text_prepare_apply_flow() -> None:
    ...
    stats = _loads(engine.stats("text"))
    ...
    assert [document["document_id"] for document in documents] == ["doc-alpha"]
    assert documents[0]["metadata"] == {"topic": "ops"}
